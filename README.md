...
5. `npm run build:selenium` will run selenium tests. To run tests locally first run `web-ext build -s extension --overwrite-dest` which builds the extenion as a zip file then run `node test/test.js`

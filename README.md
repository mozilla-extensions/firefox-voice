...
If a new browser does not open, it might be because the path to Nightly is not found. Find your application's executable: the file where Firefox Nightly can be opened manually. Enter the entire path of the file. Use the command `FIREFOX="/usr/bin/firefox" npm start` instead.

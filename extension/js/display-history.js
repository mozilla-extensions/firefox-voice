(function() {
    ...
    browser.storage.local.get("queryHistory").then((items) => {
        ...
        if (Object.keys(queryHistory).length) {
            ...
            const timeSinceLastQuery = moment.unix(mostRecentQuery.timestamp / 1000).fromNow();
            document.getElementById("most-recent-query").innerText = `${transcription} (${timeSinceLastQuery})`;

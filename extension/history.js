...
export class database {
  ...
  add(obj, TBName) {
      return new Promise((resolve, reject) => {
        ...
        request.onsuccess = (e) => {
          const database = e.target.result;
          const add = database.transaction([TBName], this.transactionModes.readwrite).objectStore(TBName).add(obj);
          add.onsuccess = event => {
  ...
  delete(primaryKey, TBName) {
    return new Promise( (resolve, reject) => {
      ...
      request.onsuccess = (e) => {
      const database = e.target.result;
      const remove = database.transaction([TBName], this.transactionModes.readwrite).objectStore(TBName).delete(primaryKey);
      remove.onerror = event => reject("This entry could not be removed from database.");
      remove.onsuccess = event => resolve("This entry has been removed from your database.");
      database.close();
    };
    });
  }
  ...
  clearAll(TBName) {
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = (e) => {
        const database = e.target.result;
        const objectStore = database.transaction(TBName, this.transactionModes.readwrite).objectStore(TBName);

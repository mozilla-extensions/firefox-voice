  ...
  add(tbName, obj) {
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = e => {
        const database = e.target.result;
        const add = database
          .transaction([tbName], Database.prototype.readwrite)
          .objectStore(tbName)
          .add(obj);
        add.onsuccess = e => {
...
Database.prototype.readonly = "readonly";
Database.prototype.readwrite = "readwrite";

Database.get = (dbName, tbName, primaryKey) => {
  return new Promise((resolve, reject) => {
 ...
 */
Database.getAll = (dbName, tbName, direction) => {
  return new Promise((resolve, reject) => {
    const request = indexedDB.open(dbName);
    request.onsuccess = e => {
      const database = e.target.result;
...
Database.delete = (dbName, tbName, primaryKey) => {
  return new Promise((resolve, reject) => {
    const request = indexedDB.open(dbName);
    request.onsuccess = e => {
      const database = e.target.result;
...
Database.clearAll = (dbName, tbName) => {
  return new Promise((resolve, reject) => {
    const request = indexedDB.open(dbName);
    request.onsuccess = e => {
      const database = e.target.result;

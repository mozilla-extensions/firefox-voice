...
registerHandler("enterText", async message => {
  const { text } = message;
  let timeout = 0;
  let el;
...
registerHandler("focusNext", async message => {
  return focusDirection(1);
});

registerHandler("formSubmit", async message => {
...
registerHandler("focusPrevious", async message => {
  return focusDirection(-1);
});

registerHandler("turnSelectionIntoLink", async message => {

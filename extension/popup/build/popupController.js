...
import * as browserUtil from "../browserUtil.js"; // eslint isn't catching the JSX that uses popupView:
...
let recorder;
let recorderIntervalId; // This is feedback that the user started, but hasn't submitted;
// if the window closes then we'll send it:

let pendingFeedback; // For tracking if the microphone works at all:

const ZERO_VOLUME_LIMIT = 5000;
let hasHadSuccessfulUtterance;
browser.storage.local.get("hasHadSuccessfulUtterance").then(results => {
...
export const PopupController = function () {
  ...
  const PERMISSION_REQUEST_TIME = 2000; // Default amount of time (in milliseconds) before the action is automatically dismissed after we perform certain actions (e.g. successfully switching to a different open tab). This value should give users enough time to read the content on the popup before it closes.
  ...
  const init = async () => {
    ...
    incrementVisits();
    backgroundTabRecorder ? await voiceShim.openRecordingTab() : await setupStream();
  ...
  const onClickLexicon = async event => {
    await browserUtil.openOrActivateTab(browser.runtime.getURL(event.target.href));
    window.close();
  };
  ...
  const handleMessage = message => {
    switch (message.type) {
      ...
      case "refreshSearchCard":
        {
          if (!message.card) {
            throw new Error(".card property missing on refreshSearchCard message");
          }
  ...
  const setPopupView = async newView => {
    ...
    if (recorder && !recorder.cancelled && !recorderIntervalId && newView === "listening") {
  ...
  const displayAutoplayFailure = () => {
    ...
    setErrorMessage("Please enable autoplay on this site for a better experience");
  };
  ...
  const startRecorder = () => {
    ...
    recorder.onEnd = json => {
      ...
      setPopupView("success");
      executedIntent = true; // Probably superfluous, since this is called in onProcessing:
  ...
  const onSubmitFeedback = async feedback => {
    ...
    pendingFeedback = feedback; // This cancels the voice input:
  ...
  return /*#__PURE__*/React.createElement(popupView.Popup, {
...
ReactDOM.render( /*#__PURE__*/React.createElement(PopupController, null), popupContainer);
...
export const OptionsController = function () {
  ...
  const [keyboardShortcutError, setKeyboardShortcutError] = useState(localStorage.getItem("keyboardShortcutError"));
  ...
  const initVersionInfo = async () => {
    setInDevelopment((await browser.runtime.sendMessage({
      type: "inDevelopment"
    })));
  ...
  const updateNickname = async (nicknameContext, oldNickname) => {
    ...
    if (nicknameContext !== undefined) {
      if (registeredNicknames[nicknameContext.nickname] !== undefined && (oldNickname === undefined || oldNickname !== nicknameContext.nickname)) {
    ...
    if (oldNickname !== undefined && (nicknameContext === undefined || oldNickname !== nicknameContext.nickname)) {
      await browser.runtime.sendMessage({
        type: "registerNickname",
        name: oldNickname,
        context: null
  ...
  const useEditNicknameDraft = (initialIsVisible, initialContext) => {
    const {
      ...
      isVisible,
      setVisible
    } = useToggle(initialIsVisible);
    const [tempEditableNickname, setTempEditableNickname] = useState({});
    const copyNickname = { ...tempEditableNickname
    };
    ...
    const setDraftVisibile = visible => {
      ...
      let intents = ""; // deep copy inital context and use that as temporary nickname for edit
  ...
  return /*#__PURE__*/React.createElement(optionsView.Options, {
...
ReactDOM.render( /*#__PURE__*/React.createElement(OptionsController, null), optionsContainer);